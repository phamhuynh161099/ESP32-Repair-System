...
// --- Cấu hình OLED (Dành cho ESP8266: SDA = D1/GPIO5, SCL = D2/GPIO4) ---
SSD1306Wire display(0x3c, 5, 4);
...
// --- Cấu hình Pin (Dành cho ESP8266) ---
#define BUZZER_PIN 14  // GPIO 14 (Chân D5)
#define LED_PIN 2      // GPIO 2 (Chân D4 - Đèn LED tích hợp)
...
const int SCROLL_SPEED = 80;         // Tốc độ cuộn (ms) - Số càng nhỏ cuộn càng nhanh
int waitTimer = 1500;                // Thời gian dừng (ms) ở đầu và cuối chu kỳ
...
void updateLED() {
  switch (currentState) {
    ...
    case STATE_ACKNOWLEDGED: digitalWrite(LED_PIN, (millis() / 500) % 2 == 0 ? LOW : HIGH); break;
    case STATE_ARRIVED: digitalWrite(LED_PIN, LOW); break;
  }
}
