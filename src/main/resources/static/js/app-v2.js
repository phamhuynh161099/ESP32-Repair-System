// API Base URL
const API_BASE = "/api/maintenance-v2";
...
function initForm() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (result.success) {
        const ticketStatus = result.data.maintenanceTracking.ticket.status;
        if (ticketStatus == "INIT") {
          showToast("Request Engineer Now In Queue!", "warn");
        } else {
          showToast("Request Engineer!", "success");
        }
        form.reset();
        lastDataJson = ""; // Ép buộc xóa cache để render lại ngay lập tức
        loadRequests();
      } else {
        showToast("Have Error, Pls Try Again!", "error");
      }
    } catch (error) {
      console.error("Error:", error);
      showToast("Can not connect server!", "error");
    }
  });
}
        ...
        <div class="request-card">
            <div class="request-header">
                <div class="machine-info">
                    ${request.ticket.machineCode} - ${request.ticket.machineName}
                </div>
            ...
            <div class="request-details">
                <p><strong><i class="fa-solid fa-map-pin"></i> Location:</strong> ${request.ticket.location}</p>
                <p><strong><i class="fa-solid fa-gears"></i> Issue:</strong> ${request.ticket.issueDescription}</p>
...
function renderTimeline(request) {
  ...
  return timestamps.length > 0
    ? `<div class="timeline">${timestamps.join("")}</div>`
    : "";
}

// Update status counts
function updateStatusCounts(requests) {
  const queue = requests.filter((r) => r.status === "NONE").length;
  const pending = requests.filter((r) => r.status === "REQUESTED").length;
  const processing = requests.filter((r) =>
    ["ACKNOWLEDGED", "ARRIVED"].includes(r.status),
  ).length;
